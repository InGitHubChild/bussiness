...
function searchPDF() {
    var params = parseQueryString(decodeURIComponent(document.location));
    var search = params.search;
    var allFieldSearch = params.allfieldsearch;
    var searchSentence = params.searchsentence;
    if(search==""){
      return;
    }
    ...
    if(!(allFieldSearch=="true"&&searchSentence!="")){
        triggerSearch(search);
      return;
    }
    var time=800;
    //为什么不写循环，然后遍历处理，因为js中没有sleep
    triggerSearch(searchSentence);
    window.setTimeout(function(){
      //获取是否有搜索出来数据，没有执行
        var findResultsCount = PDFViewerApplication.findBar.findResultsCount.innerText;
        if(findResultsCount==""){
          if(searchSentence.indexOf(" ") != -1){
              var searchSentenceTemp = searchSentence.replace(" ","");
              var sentenceArr = searchSentence.split(" ");
              triggerSearch(searchSentenceTemp);
              window.setTimeout(function(){
                  var findResultsCount = PDFViewerApplication.findBar.findResultsCount.innerText;
                  if(findResultsCount==""){
                      triggerSearch(sentenceArr[0]);
                      window.setTimeout(function(){
                          var findResultsCount = PDFViewerApplication.findBar.findResultsCount.innerText;
                          if(findResultsCount==""){
                              triggerSearch(search);
              ...
              //这也太难了:尝试了两种
              //第一种 https://github.com/lazyFurnace/blog/issues/3 不生效
              //第二种 https://ask.csdn.net/questions/337037 生效
              triggerSearch(search);
          }
        }
    },time);
...
var PDFViewerApplication = {
  ...
  load: function load(pdfDocument) {
    ...
    pdfDocument.getMetadata().then(function (_ref5) {
      ...
      if (pdfTitle) {
        _this6.setTitle(document.title);
      }
  ...
  bindEvents: function bindEvents() {
    ...
    eventBus.on('markusefuldata', webViewerMarkUsefulData);
  ...
  unbindEvents: function unbindEvents() {
    ...
    //拓展的标记有效
    eventBus.off('markUsefulData', webViewerMarkUsefulData);
    //----
...
function webViewerMarkUsefulData(){
  //获取的搜索的词，获取文件的id，获取请求的接口
    var params = parseQueryString(decodeURIComponent(document.location));
    var search = params.searchsentence;
    var markUrl = params.markurl;
    var fileId = params.fileid;
    var params = "search="+search+"&id="+fileId;
    var httpRequest = new XMLHttpRequest();//第一步：创建需要的对象
    httpRequest.open('POST', markUrl, true); //第二步：打开连接
...
function getViewerConfiguration() {
  return {
    ...
    toolbar: {
      ...
      markusefuldataButton: document.getElementById('markusefuldata'),
    ...
    secondaryToolbar: {
      ...
      markusefuldataButton: document.getElementById('secondaryMarkusefuldata'),
...
var SecondaryToolbar = function () {
  function SecondaryToolbar(options, mainContainer, eventBus) {
    ...
    this.buttons = [{
        element: options.markusefuldataButton,
        eventName: 'markusefuldata',
        close: true
    },{
...
var Toolbar = function () {
  ...
  _createClass(Toolbar, [{
    ...
    value: function _bindListeners() {
      ...
      items.markusefuldataButton.addEventListener('click', function () {
          eventBus.dispatch('markusefuldata');
      });
