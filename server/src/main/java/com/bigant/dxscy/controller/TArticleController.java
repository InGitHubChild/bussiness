package com.bigant.dxscy.controller;

import com.bigant.dxscy.model.TArticle;
import com.bigant.dxscy.service.TArticleService;
import com.github.pagehelper.PageInfo;
import io.swagger.annotations.Api;
import io.swagger.annotations.ApiOperation;
import io.swagger.annotations.ApiParam;
import org.springframework.beans.factory.annotation.Autowired;
import org.springframework.web.bind.annotation.*;

/**
 *
 */
@RestController
@RequestMapping("/tArticles")
@Api(tags = "")
public class TArticleController {
    @Autowired
    private TArticleService tArticleService;

    @GetMapping("/{id}")
    @ApiOperation("通过ID查询单个")
    public TArticle findById(@ApiParam("ID") @PathVariable("id") Integer id) {
        return tArticleService.findById(id);
    }

    @GetMapping
    @ApiOperation("分页查询")
    public PageInfo<TArticle> findByPage(@ApiParam("页号") @RequestParam(defaultValue = "1") Integer pageNum,
                                         @ApiParam("每页大小") @RequestParam(defaultValue = "10") Integer pageSize) {
        return tArticleService.findByPage(pageNum, pageSize);
    }

    @PostMapping
    @ApiOperation("新增")
    public void insert(@RequestBody TArticle tArticle) {
        tArticleService.insert(tArticle);
    }

    @PutMapping
    @ApiOperation("修改")
    public void update(@RequestBody TArticle tArticle) {
        tArticleService.update(tArticle);
    }

    @DeleteMapping("/{id}")
    @ApiOperation("通过ID删除单个")
    public void deleteById(@ApiParam("ID") @PathVariable("id") Integer id) {
        tArticleService.deleteById(id);
    }
}
